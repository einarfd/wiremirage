...
async fn list_tools_works_over_stdio_duplex() {
    ...
    assert_eq!(
        tools.len(),
        27,
        "stdio transport should expose the same 27 tools as HTTP \
         ...
         + 1 post-ADR-0021 get_capabilities + 2 ADR-0025 set_route_state/set_group_state \
         + 2 parity-batch list_journal/show_group_state)"
    );
