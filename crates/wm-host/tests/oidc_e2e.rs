...
fn config_for(issuer: &str) -> OidcConfig {
    OidcConfig {
        ...
        allow_groups: vec![],
        admin_emails: vec!["dev@acme.example".into()],
        admin_groups: vec![],
...
async fn full_callback_flow_mints_session_and_creates_user() {
    ...
    assert_eq!(body["email"], "dev@acme.example");
...
async fn github_and_oidc_logins_link_to_one_user_by_email() {
    ...
    // email the mock OIDC issuer uses ("dev@acme.example").
    ...
    async fn gh_emails() -> impl axum::response::IntoResponse {
        axum::Json(json!([
            { "email": "dev@acme.example", "primary": true, "verified": true }
        ]))
    }
    ...
    assert_eq!(me["email"], "dev@acme.example");
