...
mod tests {
    ...
    fn upsert_oauth_user_creates_then_returns_same_record() {
        let auth = fresh();
        let first = auth
            .upsert_oauth_user("github", "12345", "einar@kindly.example", false)
            .unwrap();
        assert_eq!(first.email, "einar@kindly.example");
    ...
    fn oauth_user_links_across_providers_by_verified_email() {
        ...
        let via_github = auth
            ...
            .unwrap();
        let via_oidc = auth
            .upsert_oauth_user("oidc", "sub-abc", "einar@kindly.example", false)
            .unwrap();
    ...
    fn legacy_record_backfills_email_on_login() {
        ...
        let user = auth
            .upsert_oauth_user("github", "1", "einar@kindly.example", false)
            .unwrap();
        assert_eq!(user.id, legacy_id);
        assert_eq!(user.email, "einar@kindly.example");
        assert_eq!(
            auth.get_user_by_email("einar@kindly.example")
                .unwrap()
                .unwrap()
                .id,
            legacy_id
