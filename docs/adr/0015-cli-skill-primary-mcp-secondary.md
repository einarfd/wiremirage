...
- **Streaming use cases route through MCP, not CLI.** Agents wanting to "wait until my SUT hits the mock" should use the MCP streaming tool — the agent's Bash tool doesn't handle long-running blocking commands well. *Corrected 2026-08-28:* this bullet also claimed a `wm journal tail` command "is provided for human use". No such command was ever built. The shipped CLI documentation now states its absence as deliberate — watching a stream is a push concern, served by the MCP streaming tools and the web UI's live journal — and the contributor guide records live tailing as a legitimate exception to cross-surface parity rather than a gap. The command specification has since been removed from the CLI design and recorded there as not planned, so streaming is now unqualifiedly MCP-and-UI rather than MCP-and-UI-plus-a-human-CLI-tail.
