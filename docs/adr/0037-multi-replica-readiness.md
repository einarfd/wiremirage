...
- Route create, update and delete publish an invalidation event. Subscribers reload or drop the affected route and evict its compiled-component and transpiled-JS entries. This is what makes delete and update *timely*. *Amended 2026-08-28:* the claim that a lost message degrades to the revalidation window held only for creates as originally designed — a stale delete or update still **matches**, so it never reaches the miss path the read-through lives on, and nothing bounded it. A subscriber now performs a full route-table resync on every successful subscribe, which is what makes the sentence true: a message lost to a disconnect is recovered when the subscription comes back.
...
- The originating replica receives its own event back through Valkey. This is deliberate: one uniform delivery path. Publishing both locally and to Valkey would double-deliver on the origin. *Superseded 2026-08-28 — the origin now does deliver locally as well.* One uniform path bought less than it cost: `subscribe()` returns a receiver immediately while the subscriber still has to notice demand, connect and complete a psubscribe, and anything dispatched in that window reached nobody. That window is exactly where `wait_for_request` lives, since an agent arms it *before* triggering the traffic it waits for. The origin now delivers locally and synchronously, and each published event carries the id of the journal that produced it so a subscriber drops its own — which is what prevents the double delivery this bullet was protecting against. Single-replica deployments also stop depending on pub/sub for their own traffic as a result.
...
- **Publish rides the journal write's connection.** The buffered dispatch path already performs its journal write inline, before the response returns, and the publish is issued on that same connection. *Corrected 2026-08-28:* this was written as "pipelined into one round trip, marginal cost approximately nil", and that is wrong. The sync store issues `PUBLISH` with `redis::cmd(..).query(conn)`, which waits for the reply — reusing the connection saves connection *setup*, not the round trip. Every journaled request therefore pays one additional Valkey round trip. It is a fifth round trip on a write path that already makes four, not a new network hop on an otherwise-local path, but it is not free and the ADR should not have claimed it was.
...
The functional capability actually given up is server-initiated messages outside a client request. WireMirage sends none, and the two long-running streaming tools are ordinary request-response calls that block and return. So the capability is unused, not merely tolerable to lose.

*Corrected 2026-08-28.* This paragraph originally added "there are no notifications, progress, sampling or elicitation calls anywhere in the MCP module", which is no longer true: the two streaming tools now emit progress-notification heartbeats while they block. A call that sends nothing for up to 300 seconds is indistinguishable from a hung server to any client timer or proxy read timeout between the two, so the wait had to become visibly alive. The decision is unaffected, and the distinction this paragraph draws is exactly why: progress notifications are **request-scoped**. They travel on the response stream of the request that asked for them, which puts them *inside* a client request rather than outside one. No session is involved and any replica can serve the call. The one response carrying them is switched to an event stream; every other tool keeps the plain-JSON fast path, and a caller that supplies no progress token sees no change at all.
...
The number that matters for a mock server is matched-request latency. *Corrected 2026-08-28:* this section was written asserting that it does not change, and that is wrong — a journaled request now makes one more Valkey round trip than it did. The per-bullet detail below has been corrected; this paragraph and the closing one framed the whole section around a claim the middle no longer supports.

- **Matched mock traffic (the hot path): one added Valkey round trip per journaled request.** Matching itself is unchanged — an in-process read over the cached routes. The addition is the journal publish. *Corrected 2026-08-28:* this section originally claimed "no added latency", on two premises that both turned out to be false. The publish does not pipeline into the journal write (see the Decision section above), and it is **not** skipped when nobody is tailing: a replica cannot know whether a *sibling* holds a tail, so it publishes unconditionally. Lazy subscribe bounds the cost of *receiving*, not of publishing. Whether one extra round trip per journaled request is acceptable is a live question; it was never measured before the slice shipped.
...
Most of the costs are concentrated on unmatched traffic and on active tails, both exceptional paths. The exception is the journal publish, which lands squarely on the matched request that represents the product's actual work — one extra round trip on a write path that already made four.

Worth recording for whoever revisits this: that path issues `SET` and `EXPIRE` as separate commands against the same freshly-written key, which one `SET key value EX ttl` would do in a single round trip. Collapsing that would cancel the publish exactly. So the sharper framing is not "is the publish affordable" but that this path was never round-trip-conscious to begin with, and the publish is what made that visible.
