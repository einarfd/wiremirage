...
- [0039-structural-admin-gating.md](0039-structural-admin-gating.md) — a path prefix must not imply a privilege it does not enforce: the UI's admin routes move to `/ui/admin` behind a `require_admin` router layer, replacing per-handler checks. Prompted by a self-service action found filed inside the admin-gated `/ui/settings/*` subtree, unreachable by the non-admins it was for. Leaves owner-or-admin checks (ADR-0011) and REST's `require_admin` helper alone, and is explicit that axum's lack of route enumeration makes this structural, not testable.
