...
For **outbound callbacks / webhooks** (mocking a service that calls *back* to the system-under-test — Stripe-style `charge.succeeded`, a GitHub webhook), `host.scheduleCallback({url, method, headers, body, delayMs})` hands the host a request to fire **once**, after your response is sent. It's the only network egress out of the sandbox, so it's gated two ways: the host must run with egress enabled (`WM_EGRESS=on`), and the **group must opt in** (`wm groups update <group> --callout`). If either is off, `scheduleCallback` throws (catch it). Single-attempt best-effort — no retries. The delivery outcome can't ride your response (already returned), so it lands in the group's callback journal: `wm callbacks list <group>` (and `wm callbacks show <group> <n>`), MCP `list_callbacks`, or `GET /api/groups/{group}/callbacks` — each shows `delivered` (with the SUT's status), `egress_denied`, or `failed`. See `wm capabilities callbacks` (ADR-0034). A group spec carries the opt-in as a top-level `callout: true` so import/export round-trips it.
...
`wm unmatched list` shows requests that arrived but didn't match any route — **your groups'** unmatched (an admin sees host-wide). Reach for it when debugging "my mock isn't firing". Same filter vocabulary minus `route` and `status`. It's also the **discovery surface** for reverse-engineering an SDK: let a call hit an undefined path, then `wm unmatched show <n>` (MCP: `show_unmatched`) gives the full captured envelope — method, path, headers, body — so you can build the matching route from exactly what the client sent.
