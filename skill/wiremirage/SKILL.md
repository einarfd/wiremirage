...
- **Route conflicts.** Within a group, two routes with overlapping path patterns conflict at create time (the error names the conflicting route). Conflicts are **per-group** under virtual-host routing (ADR-0030): each group is its own subdomain and path namespace, so two groups can each define the same `path`/`method` without colliding. A **catch-all** (`ANY /{path...}`, ADR-0028) is exempt — it backstops the specific routes beneath it rather than conflicting, and only collides with another catch-all that shares its prefix. At match time, specific routes win; among catch-alls, the longest prefix wins.
